...
public class HermesNodeInserter implements BatchNodeInserter {
	...
	private int tx_number=0;
	private ArrayList<PrimaryKey> keyset = new ArrayList<PrimaryKey>();

	@Override
	public void insertBatch(TGraph graph, List<TPartStoredProcedureTask> tasks,TPartStoredProcedureTask task_for_replicate) {
		...
		for(PrimaryKey key : keyset)
		{
			if(key.count/tx_number>=0.1)
			{
				key.isHotRecord=true;
			}
		}
		
		ArrayList<TPartStoredProcedureTask> tx_write_hot_record = new ArrayList<TPartStoredProcedureTask>();
		for (TPartStoredProcedureTask task : tasks)
		{
			for (PrimaryKey key : task.getWriteSet())
			{
				if(key.isHotRecord)
				{
					...
					tx_write_hot_record.add(task);
					break;
				}
			}
		}
		
		//replicate hot record
		for (int partId = 0; partId < partMgr.getCurrentNumOfParts(); partId++) {
			TxNode node = new TxNode(task_for_replicate, partId);
			node.canRoute=false;
			graph.getTxNodes().add(node);
			
			for (PrimaryKey key : keyset) {
				if(key.isHotRecord)
				{
					Node targetNode = graph.getResourcePosition(key);
					node.addReadEdges(new Edge(targetNode,key));
					targetNode.addWriteEdges(new Edge(node, key));
				}
			}
		}
		...
		for (TPartStoredProcedureTask task : tasks) {
			if(!tx_write_hot_record.contains(task))
			insertAccordingRemoteEdges(graph, task);
		}
