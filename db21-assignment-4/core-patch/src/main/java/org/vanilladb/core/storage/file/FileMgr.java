...
public class FileMgr {
	...
	private Object [] lock_array=new Object[100];
	...
	public FileMgr(String dbName) {
		...
		for(int i=0;i<100;i++)
		{
			lock_array[i]=new ReentrantReadWriteLock();
		}
	}
	 ...
	 */
	 void read(BlockId blk, IoBuffer buffer) {
		int lock_number=get_lock_number(blk.fileName());
		((ReentrantReadWriteLock) lock_array[lock_number]).readLock().lock();
		...
		try {
			...
			((ReentrantReadWriteLock) lock_array[lock_number]).readLock().unlock();
		}
	}
	 ...
	 */
	  void write(BlockId blk, IoBuffer buffer) {
		int lock_number=get_lock_number(blk.fileName());
		((ReentrantReadWriteLock) lock_array[lock_number]).writeLock().lock();
		...
		try {
			...
			((ReentrantReadWriteLock) lock_array[lock_number]).writeLock().unlock();
		}
	}
	 ...
	 */
	  BlockId append(String fileName, IoBuffer buffer) {
		int lock_number=get_lock_number(fileName);
		((ReentrantReadWriteLock) lock_array[lock_number]).writeLock().lock();
	...
	private int get_lock_number(String filename)
	{
