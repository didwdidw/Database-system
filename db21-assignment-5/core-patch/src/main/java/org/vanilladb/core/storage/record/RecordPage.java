...
public class RecordPage implements Record {
	...
	private Constant getVal(int offset, Type type) {
		...
		for (int i = tx.getSize() - 1; i >= 0 ; i--) {
			if (tx.getSize() > 0) {
				if (offsetInt.equals(tx.getOffset(i)) && blk.equals(tx.getRecordId(i).block() ))
					return tx.getModified(i);
			}
		}
	...
	private void setVal(int offset, Constant val) {
		...
		tx.addRecordId(new RecordId(blk, currentSlot));
		tx.addConstant(val);
		tx.addDoLog(Boolean.valueOf(doLog));
		tx.addOffset(Integer.valueOf(offset));
